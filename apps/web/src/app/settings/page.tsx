  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400">RAKUDAの各種設定を管理</p>
      </div>

      <div className="flex gap-6">
        {/* Sidebar */}
        <div className="w-48 flex-shrink-0">
          <nav className="space-y-1" aria-label="設定メニュー">
            {tabs.map((tab) => (
              <button
                key={tab.id}
...
function PriceSettings() {
  ...
  const { data: pricingRaw, mutate } = useSWR('/api/system-settings/category/PRICING', fetcher);
  const pricingParsed = systemSettingsCategoryResponseSchema.safeParse(pricingRaw);
  const pricingRes: { success: boolean; data: Record<string, any> } | undefined = pricingParsed.success ? pricingParsed.data : undefined;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await putApi('/api/system-settings', { settings, reason: 'UI update' });

      const isObject = (v: unknown): v is Record<string, unknown> => typeof v === 'object' && v !== null;
      if (isObject(result) && isObject((result as Record<string, unknown>).data)) {
        const dataObj = (result as Record<string, unknown>).data as Record<string, unknown>;
        const resultsUnknown = dataObj.results as unknown;
        const results: unknown[] = Array.isArray(resultsUnknown) ? resultsUnknown : [];
        for (const rr of results) {
          if (isObject(rr) && rr.success === false && rr.error === 'Not found' && typeof rr.key === 'string') {
            await postApi('/api/system-settings', {
              key: rr.key,
              value: settings[rr.key as keyof typeof settings],
              category: 'PRICING',
              valueType: 'NUMBER',
              label: rr.key,
            });
  ...
  return (
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-1.5 block text-sm font-medium text-zinc-700 dark:text-zinc-300">基本利益率 (%)</label>
              <input
                type="number"
                ...
                aria-label="eBay 基本利益率"
              />
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium text-zinc-700 dark:text-zinc-300">最低利益 ($)</label>
              <input
                type="number"
                value={Number.isNaN(ebayMinProfit) ? 0 : ebayMinProfit}
                onChange={(e) => setEbayMinProfit(Number(e.target.value))}
                className="h-10 w-full rounded-lg border border-zinc-200 bg-white px-3 text-sm outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 dark:border-zinc-700 dark:bg-zinc-900"
                aria-label="eBay 最低利益"
              />
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium text-zinc-700 dark:text-zinc-300">送料 ($)</label>
              <input
                type="number"
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-zinc-700 dark:text-zinc-300">手数料率 (%)</label>
              <input
                type="number"
                value={Number.isNaN(ebayFeeRate) ? 0 : ebayFeeRate}
                onChange={(e) => setEbayFeeRate(Number(e.target.value))}
                className="h-10 w-full rounded-lg border border-zinc-200 bg-white px-3 text-sm outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 dark:border-zinc-700 dark:bg-zinc-900"
                aria-label="eBay 手数料率"
              />
            </div>
          </div>
        </CardContent>
      ...
      <div className="flex justify-end">
        <Button onClick={handleSave} disabled={saving} aria-label="価格設定を保存">
...
function ListingSettings() {
  const settingsLinks = [
    { href: '/settings/templates', icon: Layers, title: '出品テンプレート', description: '商品タイプごとの出品設定プリセットを管理' },
    { href: '/settings/categories', icon: FolderTree, title: 'カテゴリマッピング', description: '日本語カテゴリとeBayカテゴリの紐付けを管理' },
    { href: '/settings/prompts', icon: Sparkles, title: '翻訳プロンプト', description: '商品ジャンルごとの翻訳品質を最適化' },
  ];
...
function NotificationSettings() {
  const settingsLinks = [
    { href: '/settings/notifications', icon: Bell, title: '通知チャンネル管理', description: 'Slack、Discord、LINE 通知チャンネルを設定' },
    { href: '/settings/rate-limits', icon: Shield, title: 'レート制限', description: 'スクレイピング・API呼び出しの制限を管理' },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-zinc-700 dark:text-zinc-300">価格変動通知閾値 (%)</label>
            <p className="mb-2 text-xs text-zinc-500 dark:text-zinc-400">この割合以上の価格変動があった場合に通知します</p>
            <input
              type="number"
              defaultValue={10}
              min={1}
          ...
          <div className="rounded-lg bg-amber-50 p-3 dark:bg-amber-900/20">
            <p className="text-sm text-amber-700 dark:text-amber-400">通知チャンネルを設定すると、在庫切れや価格変動を自動的に通知します。</p>
          </div>
        </CardContent>
      </Card>

      <div className="flex justify-end">
        <Button aria-label="通知設定を保存">
...
function AppearanceSettings() {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <div>
          <label className="mb-1.5 block text-sm font-medium text-zinc-700 dark:text-zinc-300">テーマ</label>
          <select
            value={theme}
          ...
          <p className="mt-1.5 text-xs text-zinc-500 dark:text-zinc-400">ヘッダーの太陽/月アイコンでも切り替えできます</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <button
              ...
              aria-pressed={theme === (t.id as 'system' | 'light' | 'dark')}
        ...
        <div>
          <label className="mb-1.5 block text-sm font-medium text-zinc-700 dark:text-zinc-300">言語</label>
          <select className="h-10 w-full rounded-lg border border-zinc-200 bg-white px-3 text-sm outline-none focus:border-amber-500 dark:border-zinc-700 dark:bg-zinc-900" aria-label="言語の選択">
