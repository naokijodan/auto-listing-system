...
type TabKey = 'dashboard' | 'syncs' | 'mappings' | 'conflicts' | 'analytics' | 'settings'
...
const API_BASE = '/api/ebay-product-catalog-synchronizer'
...
function useApi(tab: TabKey) {
  ...
  const path = useMemo(() => {
    switch (tab) {
      ...
      case 'syncs':
        return `${API_BASE}/syncs`
      case 'mappings':
        return `${API_BASE}/mappings`
      case 'conflicts':
        return `${API_BASE}/conflicts`
      case 'analytics':
...
export default function ProductCatalogSynchronizerPage(): React.ReactElement {
  const [active, setActive] = useState<TabKey>('dashboard')
  const { data, loading, error } = useApi(active)

  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-semibold text-sky-600">商品カタログ同期ツール</h1>
