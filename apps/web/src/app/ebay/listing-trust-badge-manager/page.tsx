...
type TabKey = 'dashboard' | 'listings' | 'badges' | 'verifications' | 'analytics' | 'settings';
...
const API_BASE = '/api/ebay-listing-trust-badge-manager/';

const TABS: { key: TabKey; label: string }[] = [
  { key: 'dashboard', label: 'ダッシュボード' },
  { key: 'listings', label: '出品' },
  { key: 'badges', label: 'バッジ' },
  { key: 'verifications', label: '認証' },
...
export default function Page(): JSX.Element {
  ...
  return (
    <div className="p-6 space-y-4">
      <h1 className="text-2xl font-semibold text-cyan-600">eBay 出品トラストバッジ管理</h1>
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`px-3 py-1 rounded border text-sm ${
              active === t.key ? 'bg-cyan-50 border-cyan-600 text-cyan-600' : 'bg-white border-gray-300'
            }`}
          >
            {t.label}
          </button>
