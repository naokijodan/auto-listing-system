...
const tabs = [
  ...
  { key: 'orders', label: '注文', endpoint: 'orders' },
  { key: 'carriers', label: '配送業者', endpoint: 'carriers' },
  { key: 'routes', label: 'ルート', endpoint: 'routes' },
  { key: 'analytics', label: '分析', endpoint: 'analytics' },
  { key: 'settings', label: '設定', endpoint: 'settings' },
] as const;

const apiBase = '/api/ebay-order-shipping-cost-optimizer/';

export default function OrderShippingCostOptimizerPage(): React.ReactElement {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold text-slate-600">注文配送コスト最適化</h1>
