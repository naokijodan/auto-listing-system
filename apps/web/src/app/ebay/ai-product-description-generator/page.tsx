...
const tabs = [
  ...
  { key: 'descriptions', label: '説明文', endpoint: 'descriptions/list' },
  { key: 'templates', label: 'テンプレート', endpoint: 'templates/list' },
  { key: 'styles', label: 'スタイル', endpoint: 'styles/list' },
  { key: 'analytics', label: '分析', endpoint: 'analytics' },
  { key: 'settings', label: '設定', endpoint: 'settings' },
] as const;

const apiBase = '/api/ebay-ai-product-description-generator/';

export default function AiProductDescriptionGeneratorPage(): React.ReactElement {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-red-600">AI商品説明ジェネレーター</h1>
        <p className="text-gray-500 text-sm">AI Product Description Generator</p>
      </div>
