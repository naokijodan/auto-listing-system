...
const tabs = [
  ...
  { key: 'transfers', label: '移動', endpoint: 'transfers/list' },
  { key: 'warehouses', label: '倉庫', endpoint: 'warehouses/list' },
  { key: 'routes', label: 'ルート', endpoint: 'routes/list' },
  { key: 'analytics', label: '分析', endpoint: 'analytics' },
  { key: 'settings', label: '設定', endpoint: 'settings' },
] as const;

const apiBase = '/api/ebay-inventory-transfer-manager/';

export default function InventoryTransferManagerPage(): JSX.Element {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-600">在庫移動管理</h1>
        <p className="text-gray-500 text-sm">Inventory Transfer Manager</p>
      </div>
      ...
      <div className="min-h-[160px] rounded border p-4 bg-white">
        ...
        {error && (
          <div className="text-slate-600">エラーが発生しました: {error}</div>
        )}
        {!loading && !error && (
