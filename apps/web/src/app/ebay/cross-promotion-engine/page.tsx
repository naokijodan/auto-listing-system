...
const tabs = [
  ...
  { key: 'promotions', label: 'プロモーション', endpoint: 'promotions/list' },
  { key: 'rules', label: 'ルール', endpoint: 'rules/list' },
...
const apiBase = '/api/ebay-cross-promotion-engine/';

export default function CrossPromotionEnginePage(): React.ReactElement {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-orange-600">クロスプロモーションエンジン</h1>
        <p className="text-gray-500 text-sm">Cross-Promotion Engine</p>
      </div>
      ...
      <div className="min-h-[160px] rounded border p-4 bg-white">
        ...
        {error && (
          <div className="text-orange-600">エラーが発生しました: {error}</div>
        )}
        {!loading && !error && (
