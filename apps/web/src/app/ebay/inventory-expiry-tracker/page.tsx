...
type TabKey = 'dashboard' | 'inventory' | 'expiry' | 'alerts' | 'analytics' | 'settings'

const TABS: { key: TabKey; label: string; path: string }[] = [
  { key: 'dashboard', label: 'ダッシュボード', path: 'dashboard' },
  { key: 'inventory', label: '在庫', path: 'inventory' },
  { key: 'expiry', label: '期限', path: 'expiry' },
  { key: 'alerts', label: 'アラート', path: 'alerts' },
  { key: 'analytics', label: '分析', path: 'analytics' },
  { key: 'settings', label: '設定', path: 'settings' },
]

const API_BASE = '/api/ebay-inventory-expiry-tracker/'

export default function InventoryExpiryTrackerPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-semibold text-yellow-600">在庫有効期限トラッカー</h1>
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-t-md text-sm ${
              active === t.key
                ? 'bg-yellow-50 text-yellow-700 border border-b-0 border-yellow-200'
