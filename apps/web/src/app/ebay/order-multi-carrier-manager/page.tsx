...
const tabs = [
  ...
  { key: 'orders', label: '注文', endpoint: 'orders/list' },
  { key: 'carriers', label: 'キャリア', endpoint: 'carriers/list' },
  { key: 'shipments', label: '配送', endpoint: 'shipments/list' },
  { key: 'analytics', label: '分析', endpoint: 'analytics' },
  { key: 'settings', label: '設定', endpoint: 'settings' },
] as const;

const apiBase = '/api/ebay-order-multi-carrier-manager/';

export default function OrderMultiCarrierManagerPage(): React.ReactElement {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold text-pink-600">注文マルチキャリアマネージャー</h1>
        ...
        {tabs.map((t) => (
          <button
            ...
            className={
              ...
              (active === t.key ? 'bg-pink-600 text-white' : 'bg-gray-100 hover:bg-gray-200')
            }
          >
            {t.label}
          </button>
      ...
      <div className="min-h-[160px] rounded border p-4 bg-white">
        ...
        {error && <div className="text-pink-600">エラーが発生しました: {error}</div>}
        {!loading && !error && (
