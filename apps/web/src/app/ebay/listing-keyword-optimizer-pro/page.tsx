...
const TABS = [
  ...
  { key: 'keywords', label: 'キーワード', path: 'keywords' },
  { key: 'listings', label: '出品', path: 'listings' },
  { key: 'competitors', label: '競合', path: 'competitors' },
  { key: 'analytics', label: '分析', path: 'analytics' },
  { key: 'settings', label: '設定', path: 'settings' },
]

const API_BASE = '/api/ebay-listing-keyword-optimizer-pro/'

export default function ListingKeywordOptimizerProPage() {
  ...
  return (
    <div className="p-6 space-y-4">
      <h1 className="text-2xl font-bold text-green-600">出品キーワード最適化Pro</h1>
      ...
      <div className="rounded border p-4 bg-white">
        ...
        {error && <div className="text-green-700">エラー: {error}</div>}
        {!loading && !error && (
