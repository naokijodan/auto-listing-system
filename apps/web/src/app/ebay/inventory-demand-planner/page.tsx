...
const API_BASE = "/api/ebay-inventory-demand-planner/";

const tabs = [
  { key: "dashboard", label: "ダッシュボード", path: "dashboard" },
  { key: "plans", label: "プラン", path: "plans" },
  { key: "forecasts", label: "予測", path: "forecasts" },
  { key: "scenarios", label: "シナリオ", path: "scenarios" },
  { key: "analytics", label: "分析", path: "analytics" },
  { key: "settings", label: "設定", path: "settings" },
];

export default function InventoryDemandPlannerPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold text-violet-600">在庫需要プランナー</h1>
        ...
        {tabs.map((t) => (
          <button
            ...
            className={`px-3 py-2 text-sm border-b-2 -mb-px ${
              active === t.key ? "border-violet-600 text-violet-600" : "border-transparent text-gray-600 hover:text-gray-900"
            }`}
          >
            {t.label}
          </button>
