...
const TABS: Array<{ key: string; label: string; defaultAction: string }> = [
  ...
  { key: 'reports', label: 'レポート', defaultAction: 'list' },
  { key: 'insights', label: 'インサイト', defaultAction: 'trends' },
  { key: 'forecasts', label: '予測', defaultAction: 'generate' },
...
export default function SellerBusinessIntelligencePage(): JSX.Element {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-semibold text-emerald-600 mb-4">セラービジネスインテリジェンス</h1>
        ...
        {TABS.map(tab => (
          <button
            ...
            className={`pb-2 -mb-px border-b-2 ${
              active === tab.key ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-gray-600'
            }`}
          >
            {tab.label}
          </button>
