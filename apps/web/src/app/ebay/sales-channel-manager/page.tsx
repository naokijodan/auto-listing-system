...
const tabs = [
  ...
  { key: 'channels', label: 'チャネル', endpoint: 'channels/list' },
  { key: 'products', label: '商品', endpoint: 'products/list' },
  { key: 'sync', label: '同期', endpoint: 'sync/list' },
  { key: 'analytics', label: '分析', endpoint: 'analytics' },
  { key: 'settings', label: '設定', endpoint: 'settings' },
] as const;

const apiBase = '/api/ebay-sales-channel-manager/';

export default function SalesChannelManagerPage(): React.ReactElement {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold text-green-600">販売チャネル管理</h1>
      ...
      <div className="min-h-[160px] rounded border p-4 bg-white">
        ...
        {error && (
          <div className="text-green-600">エラーが発生しました: {error}</div>
        )}
        {!loading && !error && (
