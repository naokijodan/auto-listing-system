...
type TabKey = 'dashboard' | 'payments' | 'methods' | 'disputes' | 'analytics' | 'settings';

const TABS: { key: TabKey; label: string }[] = [
  { key: 'dashboard', label: 'ダッシュボード' },
  { key: 'payments', label: '決済' },
  { key: 'methods', label: '支払方法' },
  { key: 'disputes', label: '紛争' },
  { key: 'analytics', label: '分析' },
  { key: 'settings', label: '設定' },
];

const API_BASE = '/api/ebay-order-payment-gateway';
...
function endpointForTab(tab: TabKey): string {
  switch (tab) {
    ...
    case 'payments':
      return `${API_BASE}/payments`;
    case 'methods':
      return `${API_BASE}/methods`;
    case 'disputes':
      return `${API_BASE}/disputes`;
    case 'analytics':
...
export default function OrderPaymentGatewayPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold text-orange-600">注文決済ゲートウェイ</h1>
