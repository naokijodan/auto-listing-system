...
const tabs = [
  ...
  { key: 'attributes', label: '属性', endpoint: 'attributes' },
  { key: 'pricing', label: '価格設定', endpoint: 'pricing' },
  { key: 'analytics', label: '分析', endpoint: 'analytics' },
  { key: 'settings', label: '設定', endpoint: 'settings' },
] as const;

const apiBase = '/api/ebay-product-variation-manager-pro/';

export default function ProductVariationManagerProPage(): React.ReactElement {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold text-green-600">商品バリエーション管理Pro</h1>
        ...
        {tabs.map((t) => (
          <button
            ...
            className={
              ...
              (active === t.key ? 'bg-green-600 text-white' : 'bg-gray-100 hover:bg-gray-200')
            }
          >
            {t.label}
          </button>
      ...
      <div className="min-h-[160px] rounded border p-4 bg-white">
        ...
        {error && <div className="text-green-600">エラーが発生しました: {error}</div>}
        {!loading && !error && (
