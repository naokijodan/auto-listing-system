...
const API_BASE = "/api/ebay-seller-policy-compliance-module/";
export default function Page() {
  ...
  return (<div className="p-6 space-y-6"><h1 className="text-2xl font-semibold text-violet-600">セラーポリシーコンプライアンスモジュール</h1><div className="flex gap-2 flex-wrap">{TABS.map((tab) => (<button key={tab.key} onClick={() => setActive(tab.key)} className={`px-3 py-1 rounded border text-sm ${active === tab.key ? "bg-violet-50 border-violet-600 text-violet-600" : "border-gray-300 text-gray-700 hover:bg-gray-50"}`}>{tab.label}</button>))}</div><div className="rounded border p-4"><div className="text-sm text-gray-500 mb-2">API: {API_BASE + current.path}</div>{error && <div className="text-red-600">Error: {error}</div>}{!error && !data && <div className="text-gray-500">読み込み中...</div>}{data && (<div className="space-y-1"><div><span className="font-medium">section:</span> {data.section}</div><div><span className="font-medium">action:</span> {data.action}</div></div>)}</div></div>);
}
