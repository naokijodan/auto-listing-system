...
type TabKey = 'dashboard' | 'milestones' | 'goals' | 'rewards' | 'analytics' | 'settings';
...
const API_BASE = '/api/ebay-seller-milestone-tracker/';

const TABS: { key: TabKey; label: string }[] = [
  { key: 'dashboard', label: 'ダッシュボード' },
  { key: 'milestones', label: 'マイルストーン' },
  { key: 'goals', label: '目標' },
  { key: 'rewards', label: '報酬' },
...
export default function Page(): React.ReactElement {
  ...
  return (
    <div className="p-6 space-y-4">
      <h1 className="text-2xl font-semibold text-sky-600">eBay セラーマイルストーントラッカー</h1>
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`px-3 py-1 rounded border text-sm ${
              active === t.key ? 'bg-sky-50 border-sky-600 text-sky-600' : 'bg-white border-gray-300'
            }`}
          >
            {t.label}
          </button>
