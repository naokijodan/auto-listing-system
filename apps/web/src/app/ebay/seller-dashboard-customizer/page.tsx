...
const TABS = [
  ...
  { key: 'widgets', label: 'ウィジェット', path: 'widgets' },
  { key: 'layouts', label: 'レイアウト', path: 'layouts' },
  { key: 'analytics', label: '分析', path: 'analytics' },
  { key: 'settings', label: '設定', path: 'settings' },
]

const API_BASE = '/api/ebay-seller-dashboard-customizer/'

export default function SellerDashboardCustomizerPage() {
  ...
  return (
    <div className="p-6 space-y-4">
      <h1 className="text-2xl font-bold text-slate-600">セラーダッシュボードカスタマイザー</h1>
      <div className="flex gap-2 border-b">
        {TABS.map((t) => (
          <button
            key={t.key}
            className={`px-3 py-2 text-sm border-b-2 -mb-px ${
              active === t.key ? 'border-slate-600 text-slate-600' : 'border-transparent text-gray-600 hover:text-slate-600'
            }`}
