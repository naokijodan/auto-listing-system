...
type TabKey = 'dashboard' | 'vendors' | 'scores' | 'reviews' | 'analytics' | 'settings';
...
const API_BASE = '/api/ebay-inventory-vendor-scorecard/';

const TABS: { key: TabKey; label: string }[] = [
  { key: 'dashboard', label: 'ダッシュボード' },
  { key: 'vendors', label: 'ベンダー' },
...
export default function Page(): JSX.Element {
  ...
  return (
    <div className="p-6 space-y-4">
      <h1 className="text-2xl font-semibold text-pink-600">eBay 在庫ベンダースコアカード</h1>
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`px-3 py-1 rounded border text-sm ${
              active === t.key ? 'bg-pink-50 border-pink-600 text-pink-600' : 'bg-white border-gray-300'
            }`}
          >
            {t.label}
          </button>
