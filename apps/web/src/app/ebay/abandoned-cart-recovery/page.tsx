...
const tabs = [
  ...
  { key: 'carts', label: 'カート', endpoint: 'carts/list' },
...
const apiBase = '/api/ebay-abandoned-cart-recovery/';

export default function AbandonedCartRecoveryPage(): React.ReactElement {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-indigo-600">カート放棄回復</h1>
        <p className="text-gray-500 text-sm">Abandoned Cart Recovery</p>
      </div>
      ...
      <div className="min-h-[160px] rounded border p-4 bg-white">
        ...
        {error && (
          <div className="text-indigo-600">エラーが発生しました: {error}</div>
        )}
        {!loading && !error && (
