...
const tabs = [
  ...
  { key: 'benchmarks', label: 'ベンチマーク', endpoint: 'benchmarks/list' },
  { key: 'peers', label: 'ピア', endpoint: 'peers/list' },
  { key: 'metrics', label: '指標', endpoint: 'metrics/list' },
...
export default function SellerBenchmarkingPage(): React.ReactElement {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-pink-600">セラーベンチマーキング</h1>
        <p className="text-gray-500 text-sm">Seller Benchmarking</p>
      </div>
      ...
      <div className="min-h-[160px] rounded border p-4 bg-white">
        ...
        {error && (
          <div className="text-pink-600">エラーが発生しました: {error}</div>
        )}
        {!loading && !error && (
