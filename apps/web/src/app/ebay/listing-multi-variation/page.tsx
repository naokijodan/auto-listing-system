...
const tabs = [
  ...
  { key: 'variations', label: 'バリエーション', endpoint: 'variations' },
  { key: 'options', label: 'オプション', endpoint: 'options' },
  { key: 'analytics', label: '分析', endpoint: 'analytics' },
  { key: 'settings', label: '設定', endpoint: 'settings' },
] as const;

const apiBase = '/api/ebay-listing-multi-variation/';

export default function ListingMultiVariationPage(): React.ReactElement {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-amber-600">出品マルチバリエーション管理</h1>
        <p className="text-gray-500 text-sm">Listing Multi-Variation</p>
      </div>
      ...
      <div className="min-h-[160px] rounded border p-4 bg-white">
        ...
        {error && <div className="text-amber-600">エラーが発生しました: {error}</div>}
        {!loading && !error && (
