...
const tabs = [
  ...
  { key: 'assets', label: 'アセット', endpoint: 'assets/list' },
  { key: 'collections', label: 'コレクション', endpoint: 'collections/list' },
  { key: 'uploads', label: 'アップロード', endpoint: 'uploads/list' },
  { key: 'analytics', label: '分析', endpoint: 'analytics' },
  { key: 'settings', label: '設定', endpoint: 'settings' },
] as const;

const apiBase = '/api/ebay-product-digital-asset-manager/';

export default function ProductDigitalAssetManagerPage(): React.ReactElement {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold text-fuchsia-600">商品デジタルアセット管理</h1>
        ...
        {tabs.map((t) => (
          <button
            ...
            className={
              ...
              (active === t.key ? 'bg-fuchsia-600 text-white' : 'bg-gray-100 hover:bg-gray-200')
            }
          >
            {t.label}
          </button>
      ...
      <div className="min-h-[160px] rounded border p-4 bg-white">
        ...
        {error && <div className="text-fuchsia-600">エラーが発生しました: {error}</div>}
        {!loading && !error && (
