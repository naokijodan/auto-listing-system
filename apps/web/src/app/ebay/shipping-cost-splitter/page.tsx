...
type TabKey = 'dashboard' | 'splits' | 'orders' | 'rules' | 'analytics' | 'settings'
...
const apiBase = '/api/ebay-shipping-cost-splitter'
...
export default function ShippingCostSplitterPage() {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-cyan-600">送料分割ツール</h1>
      <div className="mt-4 flex gap-2 border-b">
        {TABS.map((t) => (
          <button
            key={t.key}
            className={`px-3 py-2 text-sm border-b-2 -mb-px ${
              activeTab === t.key ? 'border-cyan-600 text-cyan-600' : 'border-transparent text-gray-600 hover:text-gray-900'
            }`}
