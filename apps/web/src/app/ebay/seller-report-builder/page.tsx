...
const tabs = [
  ...
  { key: 'sellers', label: 'セラー', endpoint: 'sellers' },
...
const apiBase = '/api/ebay-seller-report-builder/';

export default function SellerReportBuilderPage(): React.ReactElement {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-teal-600">セラーレポートビルダー</h1>
        <p className="text-gray-500 text-sm">Seller Report Builder</p>
      </div>
      ...
      <div className="min-h-[160px] rounded border p-4 bg-white">
        ...
        {error && <div className="text-teal-600">エラーが発生しました: {error}</div>}
        {!loading && !error && (
