...
const TABS: Array<{ key: string; label: string; defaultAction: string }> = [
  ...
  { key: 'inventory', label: '在庫', defaultAction: 'list' },
  { key: 'alerts', label: 'アラート', defaultAction: 'list' },
  { key: 'updates', label: '更新', defaultAction: 'latest' },
  { key: 'analytics', label: '分析', defaultAction: 'traffic' },
  { key: 'settings', label: '設定', defaultAction: 'get' },
];

const API_BASE = '/api/ebay-inventory-realtime-dashboard';

export default function InventoryRealtimeDashboardPage(): React.ReactElement {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-semibold text-lime-600 mb-4">在庫リアルタイムダッシュボード</h1>
        ...
        {TABS.map(tab => (
          <button
            ...
            className={`pb-2 -mb-px border-b-2 ${
              active === tab.key ? 'border-lime-600 text-lime-600' : 'border-transparent text-gray-600'
            }`}
          >
            {tab.label}
          </button>
