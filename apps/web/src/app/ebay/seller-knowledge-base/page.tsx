...
type TabKey = 'dashboard' | 'sellers' | 'articles' | 'categories' | 'analytics' | 'settings'
...
const API_BASE = '/api/ebay-seller-knowledge-base/'
...
export default function SellerKnowledgeBasePage(): React.ReactElement {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-semibold mb-4 text-rose-600">セラーナレッジベース</h1>
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`px-3 py-2 text-sm rounded-t ${
              active === t.key ? 'bg-rose-50 text-rose-700 border border-b-0 border-rose-200' : 'text-gray-600 hover:text-gray-800'
            }`}
          >
            {t.label}
          </button>
