...
const TABS = [
  ...
  { key: 'replenishment', label: '補充', path: 'replenishment' },
  { key: 'predictions', label: '予測', path: 'predictions' },
  { key: 'analytics', label: '分析', path: 'analytics' },
  { key: 'settings', label: '設定', path: 'settings' },
]

const API_BASE = '/api/ebay-inventory-replenishment-ai/'

export default function InventoryReplenishmentAIPage() {
  ...
  return (
    <div className="p-6 space-y-4">
      <h1 className="text-2xl font-bold text-pink-600">在庫リプレニッシュメントAI</h1>
      <div className="flex gap-2 border-b">
        {TABS.map((t) => (
          <button
            key={t.key}
            className={`px-3 py-2 text-sm border-b-2 -mb-px ${
              active === t.key ? 'border-pink-600 text-pink-600' : 'border-transparent text-gray-600 hover:text-pink-600'
            }`}
