...
type TabKey = 'dashboard' | 'listings' | 'platforms' | 'sync' | 'analytics' | 'settings'
...
const API_BASE = '/api/ebay-listing-cross-platform-sync/'
...
export default function ListingCrossPlatformSyncPage(): React.ReactElement {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-semibold mb-4 text-sky-600">出品クロスプラットフォーム同期</h1>
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`px-3 py-2 text-sm rounded-t ${
              active === t.key ? 'bg-sky-50 text-sky-700 border border-b-0 border-sky-200' : 'text-gray-600 hover:text-gray-800'
            }`}
          >
            {t.label}
          </button>
