...
const tabs = [
  ...
  { key: 'forecasts', label: '予測', endpoint: 'forecasts' },
  { key: 'models', label: 'モデル', endpoint: 'models' },
  { key: 'analytics', label: '分析', endpoint: 'analytics' },
  { key: 'settings', label: '設定', endpoint: 'settings' },
] as const;

const apiBase = '/api/ebay-inventory-forecast-engine/';

export default function InventoryForecastEnginePage(): React.ReactElement {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-rose-600">在庫フォーキャストエンジン</h1>
        <p className="text-gray-500 text-sm">Inventory Forecast Engine</p>
      </div>
      ...
      <div className="min-h-[160px] rounded border p-4 bg-white">
        ...
        {error && <div className="text-rose-600">エラーが発生しました: {error}</div>}
        {!loading && !error && (
