...
const tabs = [
  ...
  { key: 'predictions', label: '予測', endpoint: 'predictions/list' },
  { key: 'models', label: 'モデル', endpoint: 'models/list' },
  { key: 'analytics', label: '分析', endpoint: 'analytics' },
  { key: 'settings', label: '設定', endpoint: 'settings' },
] as const;

const apiBase = '/api/ebay-listing-performance-predictor/';

export default function ListingPerformancePredictorPage(): React.ReactElement {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold text-orange-600">出品パフォーマンスプレディクター</h1>
        ...
        {tabs.map((t) => (
          <button
            ...
            className={
              ...
              (active === t.key ? 'bg-orange-600 text-white' : 'bg-gray-100 hover:bg-gray-200')
            }
          >
            {t.label}
          </button>
      ...
      <div className="min-h-[160px] rounded border p-4 bg-white">
        ...
        {error && <div className="text-orange-600">エラーが発生しました: {error}</div>}
        {!loading && !error && (
