...
type TabKey = 'dashboard' | 'feedbacks' | 'trends' | 'actions' | 'analytics' | 'settings';

const TABS: { key: TabKey; label: string }[] = [
  { key: 'dashboard', label: 'ダッシュボード' },
  { key: 'feedbacks', label: 'フィードバック' },
...
function endpointForTab(tab: TabKey): string {
  switch (tab) {
    ...
    case 'feedbacks':
      return `${API_BASE}/feedbacks`;
    case 'trends':
      return `${API_BASE}/trends`;
    case 'actions':
      return `${API_BASE}/actions`;
    case 'analytics':
...
export default function SellerFeedbackAnalyzerPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold text-slate-600">セラーフィードバック分析</h1>
