  ...
  try {
    ...
    if (existing) {
      ...
      const updated = await prisma.product.update({
        ...
        data: {
          ...
          weight: data.weight,
          attributes: { ...((existing.attributes as Record<string, unknown>) || {}), shippingMethod: (data as any).shippingMethod || (existing.attributes as any)?.shippingMethod },
          sourceHash,
          scrapedAt: new Date(),
        },
      });
    ...
    const product = await prisma.product.create({
      data: {
        ...
        condition: data.condition,
        weight: data.weight,
        attributes: { shippingMethod: (data as any).shippingMethod || null },
