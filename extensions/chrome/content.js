...
(async function() {
  ...
  `;
  try {
    const modeSettings = await chrome.storage.sync.get(['exportMode']);
    const exportMode = modeSettings.exportMode || 'rakuda';
    let label = '直接エクスポート';
    if (exportMode === 'rakuda') label = 'RAKUDA に送信';
    if (exportMode === 'both') label = 'RAKUDA & Sheets に送信';
    if (exportMode === 'sheets') label = '直接エクスポート';
    exportButton.innerHTML = `
      <svg width="18" height="18" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d=\"M10 2L2 6V10C2 14.5 5 18.5 10 20C15 18.5 18 14.5 18 10V6L10 2Z\" fill=\"white\"/>
        <path d=\"M6 10L10 14L10 6M10 14L14 10\" stroke=\"${colors.primary}\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"/>
  ...
  async function initSheetSelector() {
    try {
      ...
      const syncSettings = await chrome.storage.sync.get(['spreadsheets', 'exportMode']);
      ...
      // RAKUDAのみモードでは非表示
      const exportMode = syncSettings.exportMode || 'rakuda';
      if (exportMode === 'rakuda') {
  ...
  async function exportToSpreadsheet(data, site, colors) {
    try {
      // 同期設定（スプレッドシート、画像枚数、出力先モード、RAKUDA URL）を取得
      const syncSettings = await chrome.storage.sync.get(['spreadsheets', 'imageOutputCount', 'exportMode', 'rakudaApiUrl']);
      const localSettings = await chrome.storage.local.get(['lastUsedSheetId']);
      const spreadsheets = syncSettings.spreadsheets || [];
      const imageOutputCount = typeof syncSettings.imageOutputCount === 'number' ? syncSettings.imageOutputCount : 5;
      const exportMode = syncSettings.exportMode || 'rakuda';
      const rakudaApiUrl = (syncSettings.rakudaApiUrl || 'https://api.rakuda.dev');

      // Sheetsが必要なモードなのにスプレッドシートが未登録の場合
      if ((exportMode === 'sheets' || exportMode === 'both') && spreadsheets.length === 0) {
        showNotification(
          '設定が必要です',
      ...
      // Sheets送信が必要な場合のみ選択チェック
      let selectedSheet = null;
      if (exportMode === 'sheets' || exportMode === 'both') {
      ...
      // Sheets送信
      if (exportMode === 'sheets' || exportMode === 'both') {
        console.log('📤 Sheets送信データ:', values);
        ...
        results.push({ target: 'Sheets', ...sheetResp, sheet: selectedSheet });
      }

      // RAKUDA送信
      if (exportMode === 'rakuda' || exportMode === 'both') {
        const rakudaResp = await chrome.runtime.sendMessage({
          action: 'exportToRakuda',
          apiUrl: rakudaApiUrl,
          data: data,
          platform: site
        });
        results.push({ target: 'RAKUDA', ...rakudaResp });
      }
      ...
      for (const r of results) {
        if (r.success) {
          if (r.target === 'Sheets') {
            showNotification('成功', `「${r.sheet.name}」の「${r.sheet.sheetName}」に追加しました！`, 'success', colors);
          } else {
            showNotification('成功', 'RAKUDAに送信しました！', 'success', colors);
          }
        } else {
          showNotification('エラー', `${r.target} 送信に失敗: ${r.error || '不明なエラー'}`, 'error', colors);
        }
      }
    } catch (error) {
      console.error('Export error:', error);
