...
const defaultSettings = {
  ...
  enableAmazon: true,
  exportMode: 'rakuda',
  rakudaApiUrl: 'https://api.rakuda.dev',
...
async function loadSettings() {
  try {
    ...
    const rakudaUrlEl = document.getElementById('rakudaApiUrl');
    if (exportModeEl) exportModeEl.value = syncSettings.exportMode || 'rakuda';
    if (rakudaUrlEl) rakudaUrlEl.value = syncSettings.rakudaApiUrl || 'https://api.rakuda.dev';
...
function setupEventListeners() {
  ...
  const exportModeEl = document.getElementById('exportMode');
  if (exportModeEl) {
    exportModeEl.addEventListener('change', updateModeVisibility);
  }

  // RAKUDA接続テスト
  const rakudaTestBtn = document.getElementById('rakudaTestBtn');
  if (rakudaTestBtn) {
    rakudaTestBtn.addEventListener('click', async () => {
      const apiUrl = (document.getElementById('rakudaApiUrl').value || 'https://api.rakuda.dev').trim();
      const resultEl = document.getElementById('rakudaVerifyResult');
      resultEl.style.display = 'block';
      resultEl.className = 'verification-result';
      resultEl.textContent = '接続テスト中...';
      try {
        const resp = await chrome.runtime.sendMessage({ action: 'verifyRakudaApi', apiUrl });
        if (resp && resp.success) {
          resultEl.className = 'verification-result success';
          resultEl.textContent = '✓ 接続成功';
        } else {
          resultEl.className = 'verification-result error';
          resultEl.textContent = '✗ 接続失敗: ' + (resp && resp.error ? resp.error : '不明なエラー');
        }
      } catch (e) {
        resultEl.className = 'verification-result error';
...
async function saveSettings() {
  try {
    const settings = {
      ...
      exportMode: document.getElementById('exportMode').value || 'rakuda',
      rakudaApiUrl: (document.getElementById('rakudaApiUrl').value || 'https://api.rakuda.dev').trim(),
...
function updateModeVisibility() {
  const mode = (document.getElementById('exportMode')?.value) || 'rakuda';
  const rakudaSection = document.getElementById('rakuda-settings');
  const sheetsSection = document.getElementById('sheets-section');
  if (rakudaSection) rakudaSection.style.display = (mode === 'rakuda' || mode === 'both') ? 'block' : 'none';
  if (sheetsSection) sheetsSection.style.display = (mode === 'sheets' || mode === 'both') ? 'block' : 'none';
}
