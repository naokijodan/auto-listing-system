...
## 8. Joom出品要件（担当者確認済み 2026-03-12）

### 8-1: トップ画像白背景必須
- **要件**: Joom出品時、少なくとも1枚目（トップ画像）は白抜き（白背景）が必要
- **確認元**: Joom日本事業責任者 ナスチャ（a.titova@joom.com）
- **対応方針**: 画像パイプライン（image-processor.ts）にJoom向け白背景変換ステップを追加。既存のeBay等のフローには影響させない
- **ステータス**: 未対応（21ページ完成ミッションの合間またはミッション完了後に対応）

### 8-2: ブランドオーソライズ
- **要件**: 販売予定ブランドの英語名リストをナスチャに送付すれば、Joom側でオーソライズしてくれる
- **対応方針**: DB brandsテーブル（505件）から販売予定ブランドを抽出してリスト化
- **ステータス**: 未対応
