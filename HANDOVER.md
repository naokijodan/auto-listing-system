...
## 最終更新: 2026-03-09 (Session 22: Joom画像パイプライン修正完了・検証成功)
...
## Session 22 作業内容（2026-03-09）

### Joom画像パイプライン修正 ✅

#### 修正1: WebP → JPEG変換
- `image-processor.ts`: format `'webp'` → `'jpeg'`、contentType `'image/webp'` → `'image/jpeg'`
- `joom-publish-service.ts`: format `'webp'` → `'jpeg'`、ファイル名 `.webp` → `.jpg`
- コミット: `a1d8cef4`

#### 修正2: Cloudinaryアップロード対応
- 新規: `cloudinary-uploader.ts` — Cloudinary画像アップロードモジュール（リトライ3回、MinIOフォールバック付き）
- `joom-publish-service.ts`: Cloudinary優先 → MinIOフォールバックのアップロードロジック
- 環境変数: `CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET` を.envとVPSに追加
- コミット: `a1d8cef4`

#### 修正3: Joom API v3フィールド名（根本原因）
- **根本原因**: `joom-api.ts`の`createProduct()`と`updateProduct()`でフィールド名がsnake_caseだったがJoom API v3はcamelCase必須
  - `main_image_url` → `mainImage`
  - `extra_image_urls` → `extraImages`
  - `parent_sku` → `parentSku`
- `updateProduct()`: `PUT /products/${id}` → `POST /products/update?id=${id}`
- コミット: `a182cd1f`

#### 検証結果 ✅
- 既存Joom商品（69adaf985d249b0115745542）をCloudinary URL付きで更新
- `origUrl`: 空 → Cloudinary URL設定済み、`imageState`: `pending`（Joomが画像取得処理中）
- **J1009（画像なし）infraction が消えた** — 画像修正が成功した確実な証拠
- 残存infraction: J1106（disabledByMerchant）、J1132（ブランド認証待ち）

### Joomテスト商品のクリーンアップ
| Joom Product ID | SKU | 状態 |
|-----------------|-----|------|
| 69adaf985d249b0115745542 | RAKUDA-cmmhyy6ne00021004uq1tr4o5 | disabledByMerchant（画像修正済み） |
| 69adc1b3c72ff401f8102441 | RAKUDA-SBDC081-V2 | disabledByMerchant |
| 69adc234c72ff401f8102441 | TEST-IMG-001 | 削除済み（not found） |
| 69adc26475e6b0015be45a56 | TEST-IMG-WIKI | disabledByMerchant |
| 69adc2fdc72ff401f8102441 | TEST-FIELDS | 削除済み（not found） |

### Dead-Letter-Queue修正 ✅（5キュー36件分）
| キュー | 件数 | 原因 | 修正 |
|--------|------|------|------|
| pricing-queue | 7 | price-adjustment job type未ハンドル | ✅ ハンドラ追加（コミット b23f1856） |
| inventory-queue | 8 | id undefined routing + Prismaエラー | ✅ バリデーション追加（コミット b23f1856） |
| order-queue | 5 | Prisma order.findManyエラー | ✅ include: { sales: true } 追加（コミット b23f1856） |
| scrape-queue | 4 | toLowerCase on undefined | ✅ sourceTypeバリデーション追加（コミット b23f1856） |
| image-queue | 12 | S3アップロード失敗 | ✅ リトライ+Cloudinaryフォールバック追加（コミット 4aad8679） |

### OpenAI max_tokens修正 ✅
- 全11ファイルの `max_tokens` → `max_completion_tokens` に変更
- GPT-5系モデルでの`Unsupported parameter: 'max_tokens'`エラーを解消
- コミット: `681b6d3c`

### VPSアクセス情報
- SSH: Coolify API経由で毎セッション再取得が必要: `GET /api/v1/security/keys`
- Coolify API Token: `14|85gWstw2p1iv6OtnQxwwP67NEWOqRV3xr1Uz0s4n7fafa115`
- Coolify URL: `http://45.32.28.61:8000`

### Joom一気通貫テスト成功 ✅
- Mario Party（cmmh89uzq00g9mow2we3ztmb5）でフルパイプラインテスト実施
- 画像ダウンロード(8枚) → JPEG最適化 → Cloudinaryアップロード(8枚) → Joom出品 全て成功
- Joom Product ID: `69adcef0bb18be01c685745f`（テスト後disabledByMerchant）
- 残存infraction: J1130（shipping weight required）→ camelCase修正で解決見込み、J1131（brand not found）→ Joomブランド認証が必要

### Joom APIフィールド完全camelCase化 ✅
- createProduct内の全snake_caseフィールドをcamelCase化（コミット f9526e4e）
- 対象: storeId, shippingMethod, categoryId, searchTags, shippingLength/Width/Height, dangerousKind, variant内shippingPrice/shippingWeight, addProductImageのisMain, shipOrderのtrackingNumber等
- **注意**: VPSデプロイが古いコードのまま処理された可能性あり。デプロイ後のコンテナ内コードを確認すること

### OpenAI API互換性修正 ✅
- temperatureパラメータ削除（15ファイル） — GPT-5はカスタムtemperature非対応（コミット 28daca33）
- max_tokens → max_completion_tokens修正（11ファイル）— 前セッション

### 追加テスト出品（全てdisabledByMerchant）
- Mario Party: `69adcef0bb18be01c685745f` — 画像8枚成功、J1130/J1131残存
- Splatoon 3: `69add12724862601dbd441f1` — 画像8枚成功、J1130/J1131残存
- Dragon Quest VII: `69add1a024862601dbd4462a` — 画像8枚成功

### Joomテスト商品一覧（全てdisabledByMerchant）
| Joom Product ID | 商品 |
|-----------------|------|
| 69adaf985d249b0115745542 | Seiko SBDC081（画像修正検証用） |
| 69adc1b3c72ff401f8102441 | Seiko SBDC081-V2 |
| 69adc26475e6b0015be45a56 | Test Image Wiki |
| 69adcef0bb18be01c685745f | Mario Party（一気通貫テスト） |

### J1130解消確認 ✅
- Animal Crossing テスト出品（69add36424862601dbd45810）でJ1130消滅を確認
- `shippingWeight: 0.15` がvariantに正しく設定されている
- 残存infraction: J1131（ブランド認証）のみ

### image-queue DLQ修正 ✅（12件分）
- **storage.ts**: socketTimeout 30s→120s, connectionTimeout 5s→10s, uploadFile()に3回リトライ+exponential backoff
- **image-processor.ts**: uploadToS3()にCloudinaryフォールバック（S3失敗時に自動切替、isCloudinaryConfigured()チェック付き）
- **constants.ts**: IMAGEキューのattempts 3→5, backoff delay 10s→15s
- コミット: `4aad8679`
- VPSデプロイ: Worker + API両方リスタート済み

### 次セッションの優先タスク
1. **Joomブランド認証申請** — J1131解消に必要。Joomマーチャントポータルからの認証申請（Nintendo, Seiko等）
2. **Joomテスト商品の取り下げ** — ユーザー確認後（8商品がdisabledByMerchant）
3. **image-queue DLQ 12件** — S3アップロード失敗、MinIO設定の改善 or Cloudinaryフォールバック追加
4. **category-mapper temperature修正のデプロイ確認** — Workerのtemperature削除が反映されているか（最新デプロイで修正済みのはず）
5. **出品パイプライン統合テスト** — 全チャネル（eBay含む）

---

## Session 20 作業内容（2026-03-08）

### 翻訳パイプライン完全修復（7/7成功）

#### 問題1: GPTレスポンスフォーマット不一致
- **原因**: DBデフォルトプロンプト（一般・汎用）は `Title: ...\nDescription: ...` 形式で出力を指示するが、translator.tsは `{ "translations": { "en": { "title": "..." } } }` JSON構造を期待。`response_format: json_object` 強制でGPTは `{"Title": "English Title"}` のようなJSONを返すが、コードのフォールバックで日本語原文がそのまま入る
- **修正**: GPTレスポンスの正規化コードを追加（`parsed.translations?.en?.title || parsed.Title || parsed.title || parsed.titleEn` の順で取得）
- コミット: `b85d29b2`

#### 問題2: EnrichmentTask→Product書き戻し欠落
- **原因**: `enrichment-service.ts`の`executeTask()`がEnrichmentTaskテーブルのみ更新し、Product.titleEn/descriptionEn/translationStatusを更新していなかった
- **修正**: executeTask()完了後にProduct.update()を追加
- コミット: `b85d29b2`

#### 問題3: enrichment後の画像処理ジョブ未投入
- **原因**: image-queueへのジョブ追加はscrape.tsからのみ実行され、enrichmentフローからは投入されていなかった。Chrome拡張経由の商品は画像処理されない
- **修正**: executeTask()にimage-queueジョブ投入を追加
- コミット: `a3588838`

### 画像処理パイプライン修復（7/7成功・55枚処理済み）

#### S3_ENDPOINT修正
- **問題**: S3_ENDPOINTが`http://qo4s4o04w0c8wckwk0soc44c-minio:9000`（名前逆順）でDNS解決失敗
- **修正**: `http://minio-qo4s4o04w0c8wckwk0soc44c:9000`（正しいコンテナ名）に変更
- CDN_URL=`https://minio-lw0g8gk0k4kc00cgsc8s4www.45.32.28.61.sslip.io`（バイヤーアクセス用）
- Worker/API両方を更新

#### S3クライアントタイムアウト追加
- **問題**: S3アップロードのDNS解決失敗時に無限ハング（タイムアウトなし）
- **修正**: NodeHttpHandler追加（connectionTimeout: 5s, socketTimeout: 30s）
- コミット: `bcabf1ef`

### 最終結果（Amazon JP 7商品）
| 商品 | 翻訳 | 画像 |
|------|------|------|
| スーパー マリオパーティ ジャンボリー | ✅ Nintendo Switch Super Mario Party Game | ✅ 8枚 |
| 8番出口・8番のりば | ✅ Nintendo Switch Game 8th Exit Platform 8 | ✅ 8枚 |
| スプラトゥーン3 | ✅ Splatoon 3 Multilingual Support | ✅ 8枚 |
| ドラゴンクエストVII Reimagined | ✅ Dragon Quest VII Reimagined | ✅ 8枚 |
| SPY×FAMILY OPERATION DIARY | ✅ SPY FAMILY Operation Diary | ✅ 8枚 |
| マリオカート ワールド | ✅ Mario Kart World Switch2 Game | ✅ 8枚 |
| あつまれ どうぶつの森 | ✅ Animal Crossing Game | ✅ 7枚 |

### 次セッションの優先タスク
1. **テスト出品**（Joom先行 → eBay API）
2. **出品パイプライン統合テスト**（enrichment → image → publish の一気通貫）
3. **品質ゲートの動作確認**（isLikelyEnglish(), 画像枚数チェック等）
4. **dead-letter-queue 32件の調査**

---

## Session 19 作業内容（2026-03-08）

### 本番E2Eドライラン実施（Amazon JP 8件）
- `POST /api/products/scrape` で実際のAmazon JP商品URL 8件を本番投入（出品APIは叩かない）
- **スクレイピング**: 7/8件成功（タイトル・価格・画像取得OK）
- **翻訳**: 4/7件成功、3件失敗（titleEnが日本語のまま）
- **画像処理**: 0/7件成功（processedImages全件0枚）

### 3者協議で特定した根本原因3つ
1. **翻訳の「嘘のCOMPLETED」**: 翻訳失敗時に原文をtitleEnに入れてtranslationStatus=COMPLETEDにしていた → 品質ゲートをすり抜ける
2. **品質ゲートがフラグしか見ない**: titleEnが英語かどうかを検証していなかった
3. **画像S3アップロードがタイムアウト**: WorkerコンテナからMinIOへの接続がETIMEDOUT（外部URL経由 → ヘアピンNAT問題）

### 修正実施（コミット e1309175）
1. **翻訳フォールバック修正**: 翻訳失敗時はtitleEn=空文字、translationStatus=FAILED
2. **品質ゲート強化**: isLikelyEnglish()でASCII比率チェック追加（50%未満でハードブロック）
3. **画像処理エラー明示化**: 全件失敗時のみthrow、processedImages=0ならimageStatus=ERROR
- テスト全1,331件通過

### MinIO接続修正（インフラ）
- **問題**: S3_ENDPOINT=外部URL（`https://minio-...sslip.io`）→ コンテナ内からタイムアウト
- **修正**: MinIOをCoolifyネットワークに接続（connect_to_docker_network=true）
- **修正**: S3_ENDPOINT=`http://qo4s4o04w0c8wckwk0soc44c-minio:9000`（Docker内部名）← ※名前逆順バグあり、Session 20で修正
- CDN_URLは外部URLのまま（バイヤーアクセス用）
- Worker/API両方のS3_ENDPOINTを更新、Worker再デプロイ済み
