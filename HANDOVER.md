...
## 最終更新: 2026-03-07 (Session 14: 出品パイプライン全体設計 + Phase 1実装)
...
### 出品パイプライン全体設計
- 3箇所に分散していた既存プロンプト資産を徹底調査（プロンプト例6件、プロンプト編集10+件、ItemSpecifics 3,729行）
- 2回の3者協議（Claude/GPT/Gemini）で全体設計を確定
- 設計書を `docs/LISTING_PIPELINE_DESIGN.md` に作成

### Phase 1 実装完了（commit 95afadbb）
- 設定画面にAPI連携設定カード追加（OpenAI APIキー入力・Verify・モデル選択）
- SystemSettingテーブルからAPIキーを取得（環境変数フォールバック）
- `/api/system-settings/:key/verify` エンドポイント追加
- translator.tsからロシア語翻訳を完全削除
- translator.tsのgetOpenAIClient()を非同期化しDB優先読み込みに変更

### 画像パイプライン問題の調査完了
- 2つの画像処理パイプラインが競合していることが判明
  - パイプライン①: image.ts → Product.processedImages
  - パイプライン②: ImagePipelineService → EnrichmentTask.optimizedImages
- Product.imagesが空の場合、全フォールバック失敗 → 画像なし出品
- 3者協議でパイプライン②に一本化 + Product.imagesを最終フォールバックに維持で合意

### 未完了Phase（次セッション以降）
- Phase 2: 画像パイプライン修正（最優先）
- Phase 3: translator.tsカテゴリ別プロンプト対応
- Phase 4: 主力プロンプトDB投入（時計V2、ポケカV9等）
- Phase 5: ItemSpecifics統合 + ブランド辞書DB化
- Phase 6: 品質チェックゲート

### 参照ドキュメント
- 設計書: `docs/LISTING_PIPELINE_DESIGN.md`（Phase 2-6の詳細設計・修正対象ファイル・コード例を含む）
