...
### 次セッションの優先タスク
1. **新商品でJoomテスト出品**（画像パイプライン全体の一気通貫テスト: scrape → enrich → image → Cloudinary → Joom）
2. **Joomテスト商品の取り下げ**（ユーザー確認後 — 5商品中3つがdisabledByMerchant）
3. **image-queue DLQ修正**（残り12件 — S3アップロード失敗、Cloudinary対応で改善するか確認）
4. **出品パイプライン統合テスト**（全チャネル）
