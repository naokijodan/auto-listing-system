...
### 次セッションの優先タスク
1. **camelCase修正のVPSデプロイ確認** — コンテナ内のjoom-api.jsに`shippingWeight`（camelCase）が含まれているか確認。デプロイ後テスト出品でJ1130が解消されるか検証
2. **Joomブランド認証申請** — J1131解消に必要。マーチャントポータルからNintendo/Seiko等のブランド認証
3. **Joomテスト商品の取り下げ** — ユーザー確認後（7商品がdisabledByMerchant）
4. **image-queue DLQ 12件** — S3アップロード失敗、MinIO設定の改善 or Cloudinaryフォールバック追加
5. **出品パイプライン統合テスト** — 全チャネル
