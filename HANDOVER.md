...
### Dead-Letter-Queue分析（36件）
| キュー | 件数 | 原因 |
|--------|------|------|
| image-queue | 12 | S3アップロード失敗（Cloudinary導入で改善見込み） |
| inventory-queue | 8 | id undefined routing + Prismaエラー |
| pricing-queue | 7 | price-adjustment job type未ハンドル |
| order-queue | 5 | Prisma order.findManyエラー |
| scrape-queue | 4 | toLowerCase on undefined |
...
### 次セッションの優先タスク
1. **新商品でJoomテスト出品**（画像パイプライン全体の一気通貫テスト）
2. **Joomテスト商品の取り下げ**（ユーザー確認後）
3. **dead-letter-queue修正**（pricing-queue handler, inventory-queue routing）
4. **category-mapper修正**（max_tokens → max_completion_tokens for newer OpenAI model）
5. **出品パイプライン統合テスト**（enrichment → image → Cloudinary → Joom publish）
