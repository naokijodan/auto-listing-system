...
## 最終更新: 2026-03-08 (Session 17: デプロイ完了 + インフラ安定化)
...
### seed-prompts-embedded.ts作成（Codex委託中）
- 本番DBにseedデータを投入するため、プロンプト埋め込み版のスクリプトを作成中
- ローカルファイル参照をインライン文字列に変換（VPSにローカルファイルがないため）

### 次セッションの優先タスク
1. **seed-prompts-embedded.ts本番実行**: Coolify Terminal経由で`npx tsx`実行
2. **seed-item-specifics.ts本番実行**: 同上
